...
def upload_document(
    ...
    try:
        return create_document_from_upload(db, file, conversation_id=conversation_id)
...
def read_documents(
    ...
    return list_documents(db, conversation_id=conversation_id)
...
def update_document(
    ...
    try:
        return update_document_title(db, document, payload.title)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
